...
export default function PixelOffice({ agents, activities = [], onAgentClick }: PixelOfficeProps) {
  ...
  const drawFrame = useCallback((ctx: CanvasRenderingContext2D, frame: number) => {
    ...
    // ===== PET (PIXEL THE DOG) =====
    {
      ...
      // Age and prune footprints
      for (const fp of pet.footprints) fp.age++;
      pet.footprints = pet.footprints.filter(fp => fp.age <= 60);
      // Every 2700 frames: pick a new destination
      if (frame % 2700 === 0) {
        if (Math.random() < 0.2 && agents.length > 0) {
          // 20% chance: follow nearest active agent
          let bestAnim: AnimAgent | undefined;
          let bestDist = Infinity;
          for (const ag of agents) {
            const a = animMap.get(ag.id);
            if (!a) continue;
            const d = Math.hypot(a.x - pet.x, a.y - pet.y);
            if (d < bestDist) { bestDist = d; bestAnim = a; }
          }
          if (bestAnim) {
            pet.targetX = bestAnim.x + (Math.random() - 0.5) * 20;
            pet.targetY = bestAnim.y + (Math.random() - 0.5) * 20;
          }
        } else {
          const rm = ROOMS[Math.floor(Math.random() * ROOMS.length)];
          pet.targetX = rm.x + 20 + Math.random() * (rm.w - 40);
          pet.targetY = rm.y + 20 + Math.random() * (rm.h - 40);
          pet.room = rm.id;
        }
        pet.moving = true;
      }
      // Move toward target at 0.8px/frame
      ...
      if (pdist > 3) {
        ...
        pet.y += (pdy / pdist) * 0.8;
        pet.moving = true;
        if (frame % 5 === 0) pet.footprints.push({ x: pet.x, y: pet.y, age: 0 });
      } else {
        pet.moving = false;
      }
      // Tail wag frame toggle every 20 frames
      if (frame % 20 === 0) pet.frame = pet.frame === 0 ? 1 : 0;
...
// ===== PIXEL THE DOG =====
function drawPixelDog(ctx: CanvasRenderingContext2D, pet: PetState, frameCount: number) {
  const { x, y } = pet;

  // Footprints
  for (const fp of pet.footprints) {
    const alpha = 1 - fp.age / 60;
    ctx.fillStyle = `rgba(200,134,10,${alpha})`;
    ctx.fillRect(fp.x - 2, fp.y - 1, 2, 2);
    ctx.fillRect(fp.x + 1, fp.y - 1, 2, 2);
    ctx.fillRect(fp.x - 2, fp.y + 2, 2, 2);
    ctx.fillRect(fp.x + 1, fp.y + 2, 2, 2);
  }
  ...
  // Ears: two small dark ellipses drooping slightly
  ctx.fillStyle = '#8B5E0A';
  ctx.beginPath();
  ctx.ellipse(x + 8, y - 7, 2.5, 1.5, -0.3, 0, Math.PI * 2);
  ctx.fill();
  ctx.beginPath();
  ctx.ellipse(x + 13, y - 7, 2.5, 1.5, 0.3, 0, Math.PI * 2);
  ctx.fill();

  // Tail: small arc at back, wag ±20deg
  const wagAngle = frameCount % 20 < 10 ? 20 * Math.PI / 180 : -20 * Math.PI / 180;
  ...
  ctx.moveTo(x - 8, y - 1);
  ctx.lineTo(
    x - 8 + Math.cos(Math.PI / 2 + wagAngle) * 6,
    y - 1 + Math.sin(Math.PI / 2 + wagAngle) * 6,
  );
  ctx.stroke();

  // Nose: 2px black dot at head front
  ctx.fillStyle = '#000000';
  ctx.fillRect(x + 15, y - 4, 2, 2);

  // Eyes: tiny white circle + black dot
  ...
  ctx.fillStyle = '#000000';
  ctx.beginPath();
  ctx.arc(x + 12, y - 5, 0.8, 0, Math.PI * 2);
  ctx.fill();
